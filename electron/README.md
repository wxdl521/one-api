# The One Electron Desktop App

This directory contains the Electron wrapper for The One, providing a native desktop application with system tray support for Windows, macOS, and Linux.
...
```bash
# If you have a pre-built binary (e.g., the-one-macos)
cp ../the-one-macos ../the-one
```
...
- Store database in `../data/the-one.db`
...
### Database Location
- **Development**: `../data/the-one.db` (project directory)
- **Production**:
  - macOS: `~/Library/Application Support/The One/data/`
  - Windows: `%APPDATA%/The One/data/`
  - Linux: `~/.config/The One/data/`

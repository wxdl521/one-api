...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/theone/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/theone/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/theone/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/theone/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/theone/models.json", base), fmt.Sprintf("%s/api/theone/vendors.json", base)
}

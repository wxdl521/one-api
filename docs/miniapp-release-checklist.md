...
| --- | --- | --- | --- |
| WeChat login code | The client obtains it through `wx.login` and sends it to the Mini Program BFF. The BFF exchanges it with WeChat's `code2Session` endpoint. | Request memory only; it is not persisted. | Sent only to the configured BFF and WeChat. Do not log the code. |
...
The Mini Program does not request profile, phone-number, location, contacts, camera, microphone, photo-album, or user-info permissions. It does not include a client analytics SDK or call a third-party API directly. Its application API client targets only the configured HTTPS BFF; browser handoffs use only the verified console origin, while WeChat login and authorised model-provider calls occur server-side.

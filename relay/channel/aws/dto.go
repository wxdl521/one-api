...
func convertToNovaRequest(req *dto.GeneralOpenAIRequest) *NovaRequest {
	...
	// 采样参数（temperature/topP/topK）指针直通保留客户端显式 0（0 在各自取值域内有效）；
	// max_tokens=0 无意义（Nova 要求 >=1），按未设置处理不下发，交由上游取默认，避免 400。
	if (req.MaxTokens != nil && *req.MaxTokens != 0) || req.Temperature != nil || req.TopP != nil || req.TopK != nil || req.Stop != nil {

...
import (
	...
	"github.com/QuantumNous/the-one/relay/channel/ai360"
	"github.com/QuantumNous/the-one/relay/channel/lingyiwanwu"
	"github.com/QuantumNous/the-one/relaykit/dto"

	//"github.com/QuantumNous/the-one/relay/channel/minimax"
	"github.com/QuantumNous/the-one/relay/channel/openrouter"
	"github.com/QuantumNous/the-one/relay/channel/xinference"
	relaycommon "github.com/QuantumNous/the-one/relay/common"
	"github.com/QuantumNous/the-one/relay/common_handler"
	...
	"github.com/QuantumNous/the-one/setting/reasoning"
...
func (a *Adaptor) GetRequestURL(info *relaycommon.RelayInfo) (string, error) {
	...
	case constant.ChannelTypeAzure:
		...
		if info.RelayMode == relayconstant.RelayModeRealtime {
			if strings.Contains(model_, "-realtime-preview") {
				requestURL = fmt.Sprintf("/openai/realtime?deployment=%s&api-version=%s", url.QueryEscape(model_), apiVersion)
			} else {
				requestURL = fmt.Sprintf("/openai/v1/realtime?model=%s", url.QueryEscape(info.UpstreamModelName))
			}
		}
		return relaycommon.GetFullRequestURL(info.ChannelBaseUrl, requestURL, info.ChannelType), nil
...
func (a *Adaptor) SetupRequestHeader(c *gin.Context, header *http.Header, info *relaycommon.RelayInfo) error {
	...
	if info.ChannelType == constant.ChannelTypeOpenRouter {
		if header.Get("HTTP-Referer") == "" {
			header.Set("HTTP-Referer", "https://www.theone.ai")
		}
		if header.Get("X-OpenRouter-Title") == "" {
			header.Set("X-OpenRouter-Title", "The One")
		}
	}
	return nil
}
...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.TheOneError) {
	switch info.RelayMode {

...
func TextHelper(c *gin.Context, info *relaycommon.RelayInfo) (theOneError *types.TheOneError) {
	...
	if info.RelayMode == relayconstant.RelayModeChatCompletions &&
		...
		usage, theOneErr := chatCompletionsViaResponses(c, info, adaptor, request)
		if theOneErr != nil {
			return theOneErr
		}
	...
	usage, theOneErr := adaptor.DoResponse(c, httpResp, info)
	if theOneErr != nil {
		// reset status code 重置状态码
		service.ResetStatusCode(theOneErr, statusCodeMappingStr)
		return theOneErr
